...
import (
	...
	purgeK8sJobs "github.com/azharullah/purgek8sjobs"
)

var cmdDescription = "This utility helps purge all kubernetes jobs that have " +
	"completed their execution more than `b` hours ago in the namespace `n`. " +
	"Optionally, this also write the job spec and events to a provided log file path"

// rootCmd represents the base command when called without any subcommands
var rootCmd = &cobra.Command{
	Use: "purge-completed-k8s-jobs --namespace xyz --before-hours 5" +
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		resp := purgeK8sJobs.PurgeJobs(namespace, beforeHours, options)
		if resp.Success {
			logrus.Infof("Completed execution. Summary: \n%v", resp.Msg)
		} else {
